...
export default function Dashboard({ name, title, unitSelectionValues, goal }) {
  ...
  useEffect(() => {
    ...
    const dataRef = ref(db, `/millCreek/behaviours/2025/${months_backword[desiredMonth]}`);
  ...
  useEffect(() => {
    const yearsRef = ref(db, `/millCreek/behaviours`);
    console.log('Checking available years/months for Mill Creek');
      ...
      <div className={styles['chart-container']}>
        <div className={styles.chart}>
          ...
          >
            ...
            <option value="hour">By Hour (24hr)</option>on>
          </select>
