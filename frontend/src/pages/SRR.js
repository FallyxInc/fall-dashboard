...
export default function Dashboard({ name, title, unitSelectionValues, goal }) {
  console.log('Dashboard component mounted. Name prop:', name);
  ...
  console.log('Initial desiredYear state:', desiredYear);
  ...
  const updateAnalysisChart = () => {
    ...
    switch (analysisType) {
      case 'timeOfDay':
        ...
        console.log('[SRR.js] Data being sent to countFallsByTimeOfDay:', filteredData);
        console.log('[SRR.js] Name prop being sent to countFallsByTimeOfDay:', name);
      ...
      complete: async (results) => {
        try {
          ...
          for (let i = 0; i < newData.length; i++) {
            ...
            const rowRef = ref(db, `srr/${desiredYear}/${months_backword[desiredMonth]}/${i}`);
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      pastThreeMonths.forEach(({ year, month }) => {
        ...
        // but we'll include a cleanup for all three-month listeners at the end of useEffect
      });
  ...
  return (
    <div className={styles.dashboard} ref={tableRef}>
      <h1>Scarborough Retirement Residence Falls Dashboard</h1>
          ...
          {data.map((item, i) => {
            ...
            return (
              <tr key={item.id} style={{ /* backgroundColor: isHighlighted ? 'lightgray' : 'inherit' */ }}>
                ...
                <td style={{ fontSize: '16px', width: '300px', backgroundColor: item.isCauseUpdated === 'yes' ? 'rgba(76, 175, 80, 0.3)' : 'inherit' }}>
                  {item.cause}
                  <br />
                ...
                <td
                  style={{
                    fontSize: '16px',
                    width: '300px',
                    backgroundColor: item.isInterventionUpdated === 'yes' ? 'rgba(76, 175, 80, 0.3)' : 'inherit'
                  }}
                >
                  {item.intervention}
                </td>
