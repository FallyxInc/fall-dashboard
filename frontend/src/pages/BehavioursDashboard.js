  ...
  useEffect(() => {
    ...
    onValue(yearsRef, (snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        // Find the closest month to today that has data
        const today = new Date();
        const currentYear = today.getFullYear().toString();
        const currentMonth = getCurrentMonth();
        
        // Start with current year and month
        let yearToUse = currentYear;
        let monthToUse = currentMonth;
        
        // If current year/month doesn't have data, look for closest available
        if (!sortedMapping[yearToUse]?.includes(monthToUse)) {
          // First try to find a month in current year
          if (sortedMapping[yearToUse]?.length > 0) {
            // Find closest month in current year
            ...
            // If no data in current year, use most recent year
            yearToUse = sortedYears[0];
            // Use most recent month in that year
